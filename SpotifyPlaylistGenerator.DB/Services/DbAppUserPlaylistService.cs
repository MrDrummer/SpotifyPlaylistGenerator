...
public class DbAppUserPlaylistService : IDbAppUserPlaylistService
{
    ...
    {
        // TODO: USE AddOrUpdate INSTEAD
        await using var transaction = await _context.Database.BeginTransactionAsync();

        var dbPlaylists = playlists.ToList();
        var playlistIdsToCheck = dbPlaylists.Select(pl => pl.Id).ToList();
        var existingPlaylistIds = await _context.Playlists
            .Where(p => playlistIdsToCheck.Contains(p.Id))
            .Select(p => p.Id)
            .ToListAsync();

        var playlistsToAdd = dbPlaylists.Where(pl => !existingPlaylistIds.Contains(pl.Id)).ToList();
        ...
        var existingJunctions = await _context.AppUserPlaylists
            .Where(aup => aup.AppUserId == appUser.Id && playlistIdsToCheck.Contains(aup.PlaylistId))
            .ToListAsync();

        
        // TODO: USE AddOrUpdate INSTEAD
        var junctionsToAdd = dbPlaylists
